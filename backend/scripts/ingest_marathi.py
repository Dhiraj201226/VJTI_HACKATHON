...
def ingest_pipeline():
    print("=" * 60)
    logger.info("Starting Marathi Half Sync ingestion pipeline...")

    # Set custom dataset path for this script
    os.environ["DATASET_PATH"] = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 
        "mahGRs", "GR_combine", "marathi_half_sync.txt"
    )

    BASE_DIR = Path(__file__).parent.parent
    progress_file = BASE_DIR / "data" / "ingest_marathi_progress.txt"
    start_from_gr = 0
